package com.example.wait.democopsboot.util.test;
...
public class ConstraintViolationSetAssert extends
	...
	public ConstraintViolationSetAssert(
	...
	public ConstraintViolationSetAssert hasViolationSize(
			int numberOfViolations) {
		isNotNull();

		if (actual.size() != numberOfViolations) {
			failWithMessage("Expected %s violations, but got %s",
					numberOfViolations, actual.size());
		}
	...
	public ConstraintViolationSetAssert hasNoViolations() {
		...
		if (!actual.isEmpty()) {
			failWithMessage(
					"Expecting no violations, but there are %s violations",
					actual.size());
		}
	...
	private boolean containsViolationWithPath(
			Set<? extends ConstraintViolation> violations, String path) {
